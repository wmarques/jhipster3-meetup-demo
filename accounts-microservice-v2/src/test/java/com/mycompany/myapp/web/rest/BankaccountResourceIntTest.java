...
import com.mycompany.myapp.AccountsMicroservicev2App;
...
@SpringApplicationConfiguration(classes = AccountsMicroservicev2App.class)
@WebAppConfiguration
@IntegrationTest
public class BankaccountResourceIntTest {
    ...
    public void searchBankaccount() throws Exception {
        ...
        restBankaccountMockMvc.perform(get("/api/_search/bankaccounts?query=id:" + bankaccount.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bankaccount.getId().intValue())))
            .andExpect(jsonPath("$.[*].clientname").value(hasItem(DEFAULT_CLIENTNAME.toString())))
